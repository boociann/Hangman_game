...
def f_mask_word(word):
    masked_rand_word = [' ' if i == " " else "_" for i in list(word)] #didn't need to convert to string, as they are iterable
...
def f_print_actual_board(board, used_letters, lives):
    ...
    print('\nThe word to guess is: ' ,*board, sep= ' ') # check what * does
...
def f_repeat():
    if lives <= 0 or complete == 1:
        ...
        if rep == "Y":
            ...
            return 0
    # instead 0 an 1 use Boolen - simplifies and brings clarity
...
def f_give_a_clue(country,lives):
    ...
    if lives <= 3:
        print(f"As it's {current_day} today, we will offer you a clue, if you type \"Y\".")
